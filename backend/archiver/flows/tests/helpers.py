...
def expected_job_status(job_type: str, status: SciCatClient.STATUSMESSAGE) -> Dict[str, Any]:
    match status:
        case SciCatClient.STATUSMESSAGE.FINISHED_UNSUCCESSFULLY:
            return Job(jobStatusMessage="finishedUnsuccessful").model_dump(exclude_none=True)
        case SciCatClient.STATUSMESSAGE.FINISHED_WITHDATASET_ERRORS:
            return Job(jobStatusMessage="finishedWithDatasetErrors").model_dump(exclude_none=True)
        case SciCatClient.STATUSMESSAGE.IN_PROGRESS:
            return Job(jobStatusMessage="inProgress").model_dump(exclude_none=True)
...
def expected_jobresultsobject(dataset_id: str, datablocks: List[DataBlock]):
    ...
    for result in results:
        dataset_to_datablocks.setdefault(dataset_id, []).append(
