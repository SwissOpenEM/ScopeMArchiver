...
def test_s3_interface(s3):
    ...
    presigned_url = s3.get_presigned_url(bucket=bucket, filename=filename)
    assert filename in presigned_url

    listed_objects = s3.list_objects(bucket=bucket, folder="")

    assert len(listed_objects) == 1
    assert listed_objects[0].Name == filename

    stat_object = s3.stat_object(bucket=bucket, filename=filename)
    ...
    with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
        ...
        stat_object = s3.stat_object(bucket=bucket, filename=tmp_file.name)
