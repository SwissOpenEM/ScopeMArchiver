...
class BaseS3UploadApiImpl(BaseS3uploadApi):
    ...
    async def finalize_dataset_upload(
        ...
        try:
            await mark_dataset_as_archivable(finalize_dataset_upload_body.dataset_id)
    ...
    async def request_dataset_upload(
        self, upload_request_body: UploadRequestBody
    ) -> UploadRequestSuccessfulResp | UploadRequestUnsuccessfulResp:
        return await request_upload(upload_request_body.dataset_id, upload_request_body.dataset_size_gib)

    async def abort_dataset_upload(
        self, abort_dataset_upload_body: AbortDatasetUploadBody
    ) -> AbortDatasetUploadResp:
        return await abort_upload(abort_dataset_upload_body.dataset_id)
